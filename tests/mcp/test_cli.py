...
def test_default_command_is_serve(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with (
        patch("robin_stocks_mcp.server._cmd_serve", return_value=0) as serve,
...
def test_tda_setup_generates_passcode_when_flag_given(capsys) -> None:
    with (
        patch("robin_stocks.tda.generate_encryption_passcode", return_value="GEN-PASS"),
...
def test_logout_clears_both_brokers_by_default() -> None:
    with patch("robin_stocks.robinhood.logout") as rh_out, patch("robin_stocks.gemini.logout") as gem_out:
        main(["logout"])
