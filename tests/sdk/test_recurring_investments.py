...
def test_create_recurring_investment_happy_path() -> None:
    with patch("robin_stocks.robinhood.recurring_investments.load_account_profile", return_value={"account_number": "ACC-1"}), \
...
def test_create_recurring_investment_uses_explicit_account_number() -> None:
    ...
    with patch("robin_stocks.robinhood.recurring_investments.load_account_profile") as lap, \
         patch("robin_stocks.robinhood.recurring_investments.get_instruments_by_symbols", return_value=[{"id": "inst-1"}]), \
...
def test_update_recurring_investment_builds_correct_patch_payload() -> None:
    ...
    with patch("robin_stocks.robinhood.recurring_investments.SESSION", mock_session):
        out = ri.update_recurring_investment(
            "sched-1", account_number="ACC-1", state="paused", amount=10.0
        )
...
def test_update_recurring_investment_jsonify_false_returns_raw_response() -> None:
    ...
    with patch("robin_stocks.robinhood.recurring_investments.SESSION", mock_session):
        out = ri.update_recurring_investment(
            "s1", account_number="ACC", state="paused", jsonify=False
        )
    assert out is resp
...
def test_update_recurring_investment_with_all_optional_fields() -> None:
    ...
    with patch("robin_stocks.robinhood.recurring_investments.SESSION", mock_session):
        ri.update_recurring_investment(
            "s1", account_number="ACC",
            amount=20.0, frequency="monthly",
            state="active", start_date="2026-12-15",
        )
...
def test_get_next_investment_date_defaults_start_date_to_today() -> None:
    today = datetime.now().strftime("%Y-%m-%d")
    with patch("robin_stocks.robinhood.recurring_investments.request_get", return_value={"next_investment_date": "2026-05-27"}) as rg:
...
def test_get_next_investment_date_explicit_start_date() -> None:
    with patch("robin_stocks.robinhood.recurring_investments.request_get", return_value={"next_investment_date": "2026-06-08"}) as rg:
