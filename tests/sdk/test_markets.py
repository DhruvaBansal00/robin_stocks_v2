...
def test_get_all_stocks_from_market_tag_returns_quotes() -> None:
    with patch("robin_stocks.robinhood.markets.request_get", return_value={"instruments": ["url1"]}), \
...
def test_get_market_today_hours_resolves_market() -> None:
    with patch("robin_stocks.robinhood.markets.get_markets",
              return_value=[{"mic": "XNYS", "todays_hours": "https://x/hours/"}]), \
...
def test_get_market_today_hours_invalid_market_raises() -> None:
    with patch("robin_stocks.robinhood.markets.get_markets",
              return_value=[{"mic": "XNYS", "todays_hours": "url"}]):
...
def test_get_market_next_open_hours_resolves_market() -> None:
    ...
    with patch("robin_stocks.robinhood.markets.get_market_today_hours", return_value="https://x/next/"), \
         patch("robin_stocks.robinhood.markets.request_get", return_value={"opens_at": "9:30"}):
...
def test_get_market_next_open_hours_after_date_resolves() -> None:
    with patch("robin_stocks.robinhood.markets.get_market_hours",
              return_value={"next_open_hours": "https://x/after/"}), \
         patch("robin_stocks.robinhood.markets.request_get", return_value={"opens_at": "9:30"}):
