...
def test_option_positions_url_with_account() -> None:
    assert (
        urls.option_positions_url("ABC-123")
        == "https://api.robinhood.com/options/positions/?account_numbers=ABC-123"
    )


def test_option_positions_url_with_keyword_arg() -> None:
    assert (
        urls.option_positions_url(account_number="X")
        == "https://api.robinhood.com/options/positions/?account_numbers=X"
    )
...
def test_option_orders_url_with_order_id() -> None:
    assert (
        urls.option_orders_url(orderID="abc")
        == "https://api.robinhood.com/options/orders/abc/"
    )


def test_option_orders_url_with_account_only() -> None:
    assert (
        urls.option_orders_url(account_number="ACC")
        == "https://api.robinhood.com/options/orders/?account_numbers=ACC"
    )
...
def test_option_orders_url_with_account_and_start_date_uses_and_separator() -> None:
    ...
    assert out == (
        ...
        "?account_numbers=ACC&updated_at[gte]=2026-01-01"
    )


def test_option_orders_url_with_all_three() -> None:
    out = urls.option_orders_url(
        orderID="ord", account_number="ACC", start_date="2026-01-01"
    )
    assert out == (
        "https://api.robinhood.com/options/orders/ord/"
        "?account_numbers=ACC&updated_at[gte]=2026-01-01"
    )
...
def test_tax_lots_open_url() -> None:
    assert (
        urls.tax_lots_open_url("ACC-123", "INST-456")
        == "https://api.robinhood.com/tax_lots/open/ACC-123/INST-456/"
    )


def test_tax_lots_selected_url() -> None:
    assert (
        urls.tax_lots_selected_url("order-xyz")
        == "https://api.robinhood.com/tax_lots/order/order-xyz/selected/"
    )


def test_tax_lots_closed_url() -> None:
    assert (
        urls.tax_lots_closed_url("order-xyz")
        == "https://api.robinhood.com/tax_lots/order/order-xyz/closed/"
    )
...
def test_marketdata_index_quotes_url() -> None:
    ...
    assert (
        urls.marketdata_index_quotes_url("idx-id-1")
        == "https://api.robinhood.com/marketdata/indexes/values/v1/idx-id-1/"
    )
...
def test_marketdata_quotes_url_unaffected_by_index_addition() -> None:
    ...
    assert (
        urls.marketdata_quotes_url("inst-1")
        == "https://api.robinhood.com/marketdata/quotes/inst-1/"
    )
...
def test_futures_contract_url() -> None:
    assert (
        ...
        == "https://api.robinhood.com/arsenal/v1/futures/contracts/symbol/ESH26"
    )
...
def test_futures_quotes_url() -> None:
    assert (
        urls.futures_quotes_url()
        == "https://api.robinhood.com/marketdata/futures/quotes/v1/"
    )


def test_futures_orders_url() -> None:
    assert (
        urls.futures_orders_url("acct-uuid")
        == "https://api.robinhood.com/ceres/v1/accounts/acct-uuid/orders"
    )
...
def test_recurring_schedules_url_by_schedule_id_takes_priority() -> None:
    ...
    out = urls.recurring_schedules_url(
        schedule_id="sched-1", account_number="should-be-ignored"
    )
...
def test_recurring_schedules_url_by_account_number() -> None:
    ...
    assert out == (
        "https://bonfire.robinhood.com/recurring_schedules/?account_number=ACC-1"
    )
...
def test_recurring_schedules_url_with_asset_types_list() -> None:
    ...
    assert out == (
        "https://bonfire.robinhood.com/recurring_schedules/"
        "?asset_types=equity&asset_types=crypto"
    )
...
def test_recurring_schedules_url_with_single_asset_type_string() -> None:
    ...
    assert out == (
        "https://bonfire.robinhood.com/recurring_schedules/?asset_types=equity"
    )
...
def test_recurring_schedules_url_asset_types_wins_over_account_number() -> None:
    ...
    out = urls.recurring_schedules_url(
        account_number="ACC-1", asset_types=["equity"]
    )
...
def test_next_investment_date_url() -> None:
    ...
    assert out == (
        "https://bonfire.robinhood.com/recurring_schedules/equity/"
        "next_investment_date/?frequency=weekly&start_date=2026-05-20"
    )
