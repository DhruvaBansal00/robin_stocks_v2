...
def test_validate_sherrif_id_prompt_challenge_validated(no_sleep) -> None:
    ...
    inquiry_with_prompt = {
        "context": {"sheriff_challenge": {"type": "prompt", "status": "issued", "id": "ch-1"}}
    }
    prompt_validated = {"challenge_status": "validated"}
    workflow_approved = {"type_context": {"result": "workflow_status_approved"}}

    with patch("robin_stocks.robinhood.authentication.request_post",
              side_effect=[machine_response, workflow_approved]) as rp, \
         patch("robin_stocks.robinhood.authentication.request_get",
              side_effect=[inquiry_with_prompt, prompt_validated]):
...
def test_validate_sherrif_id_sms_challenge(no_sleep) -> None:
    ...
    machine_response = {"id": "machine-1"}
    inquiry_sms = {
        "context": {"sheriff_challenge": {"type": "sms", "status": "issued", "id": "ch-1"}}
    }
    workflow_approved = {"type_context": {"result": "workflow_status_approved"}}
...
def test_validate_sherrif_id_validated_status_breaks_loop(no_sleep) -> None:
    ...
    machine_response = {"id": "machine-1"}
    inquiry_validated = {
        "context": {"sheriff_challenge": {"type": "other", "status": "validated", "id": "ch-1"}}
    }
    workflow_approved = {"type_context": {"result": "workflow_status_approved"}}

    with patch("robin_stocks.robinhood.authentication.request_post",
              side_effect=[machine_response, workflow_approved]), \
         patch("robin_stocks.robinhood.authentication.request_get",
              side_effect=[inquiry_validated]):
        auth._validate_sherrif_id("device-tok", "workflow-1")
...
def test_validate_sherrif_id_workflow_status_field_approves(no_sleep) -> None:
    ...
    with patch("robin_stocks.robinhood.authentication.request_post",
              side_effect=[machine_response, workflow_via_status]), \
         patch("robin_stocks.robinhood.authentication.request_get",
              side_effect=[inquiry_validated]):
        auth._validate_sherrif_id("device-tok", "workflow-1")
