...
withdrawals = sum(
    float(x["amount"]) for x in allTransactions if (x["direction"] == "withdraw") and (x["state"] == "completed")
)
debits = sum(
...
print(f"The net worth has increased {percentDividend:0.2}% due to dividends that amount to {dividends:0.2f}")
print(f"The net worth has increased {percentGain:0.3}% due to other gains that amount to {totalGainMinusDividends:0.2f}")
