...
async def rh_export_completed_stock_orders(
    dir_path: str, file_name: str | None = None, account_number: str | None = None
) -> str:
    """Write all completed stock orders to a CSV in dir_path."""
    await to_thread(rh.export_completed_stock_orders, dir_path, file_name=file_name, account_number=account_number)
    return f"ok: stock orders exported to {dir_path}"


@mcp.tool()
@safe_tool(write=True)
async def rh_export_completed_crypto_orders(dir_path: str, file_name: str | None = None) -> str:
...
@mcp.tool()
@safe_tool(write=True)
async def rh_export_completed_option_orders(dir_path: str, file_name: str | None = None) -> str:
