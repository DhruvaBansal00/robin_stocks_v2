...
def _validate_sherrif_id(device_token: str, workflow_id: str):
    ...
    while time.time() - start_time < 120:  # 2-minute timeout
        try:
            ...
            retry_attempts -= 1
            if retry_attempts == 0:
                raise TimeoutError("Max retries reached. Assuming login approved and proceeding.") from e
            ...
            try:
                with open(pickle_path) as f:
                    ...
                    return {
                        ...
                        "expires_in": expiresIn,
                        "scope": scope,
                        "detail": f"logged in using authentication in {creds_file}",
                        "backup_code": None,
                        "refresh_token": refresh_token,
                    }
            except Exception:
                print(
                    "ERROR: There was an issue loading session file. Authentication may be expired - logging in normally.",
                    file=get_output(),
                )
