...
def get_open_crypto_positions(info=None):
    ...
    :param info: Will filter the results to get a specific value.
    :type info: Optional[str]
    :returns: [list] Returns a list of dictionaries of key/value pairs for each open position. If info parameter \
...
def get_crypto_historicals(symbol, interval="hour", span="week", bounds="24_7", info=None):
    ...
    if interval not in interval_check:
        print('ERROR: Interval must be "15second","5minute","10minute","hour","day",or "week"', file=get_output())
        return [None]
